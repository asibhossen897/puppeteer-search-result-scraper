...
(async () => {
  ...
  await page.type('input[id="q"]', 'puppeteer tutorials');
  ...
  const results = await page.evaluate(() => {
    const items = document.querySelectorAll('div.result');   // Main div
    return Array.from(items).map(item => {
      const titleElement = item.querySelector('h2');  // Search title
      const linkElement = item.querySelector('a.result-link[href]');  // Site link
